...
exp.runSentBillingToday = () => {
    return new Promise(async (resolve, reject) => {
        ...
        const dataTalent = await query(`SELECT * FROM (SELECT *, (SELECT batch FROM bootcamp.batch WHERE id_batch = talent.id_batch) AS real_batch FROM hiring.talent WHERE  status_talent = 'work' AND status_payment IN ('active', 'paycut') AND email IN('agungwar00@gmail.com', 'alfa.putrakurnia@gmail.com', 'alden.hari@gmail.com')) AS data WHERE real_batch = '8'`, [], 'arkademy').then(result => {
            return result.rows
        }).catch(error => {
            return reject(error)
        })
